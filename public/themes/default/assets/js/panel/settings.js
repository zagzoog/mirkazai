...
function groqSettingsSave() {
    ...
    var formData = new FormData();
    formData.append('groq_api_key', $("#groq_api_key").val());

    $.ajax({
        type: "post",
        url: "/dashboard/admin/settings/groq-save",
        data: formData,
        contentType: false,
        processData: false,
        success: function (data) {
            toastr.success(magicai_localize?.settings_saved || 'Settings saved successfully');
            document.getElementById("settings_button").disabled = false;
            document.getElementById("settings_button").innerHTML = "Save";
        },
        error: function (data) {
